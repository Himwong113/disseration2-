...
#
        ...
        #x = self.conv5(x)
        #print(f'x = {x.shape} ')
        # print(f'x.cat shape ={x.shape}') #x.cat shape =torch.Size([32, 512, 1024])
        x = torch.cat((x1, x2, x3), dim=1)
        x = torch.permute(x, (0, 2, 1))

        #print(f'x = {x.shape} ')
        # x = torch.permute(x,(0,2,1))
        x = F.relu(self.ln8( self.linear(x)))
        #
        #print(f'finial output ={x.shape}')
        return x
class LatentCapsLayer(nn.Module):
    def __init__(self, latent_caps_size=40, prim_caps_size=1024, prim_vec_size=40, latent_vec_size=40):
...
class DGCNNCaps(nn.Module):
    def __init__(self,args, output_channel =40):
        super(DGCNNCaps,self).__init__()
    ...
    def forward(self,data):
        ...
        print(f'result ={result.shape}') #batch *32
