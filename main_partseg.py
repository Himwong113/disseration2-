...
from model_partseg import DGCNNCaps_part
import numpy as np
...
def train(args, io):
    ...
    if args.model == 'dgccaps_part':
        model = DGCNNCaps_part(args, output_channel=40,seg_part=seg_num_all).to(device)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in tqdm.tqdm( train_loader):
            ...
            loss = criterion(seg_pred.view(-1, seg_num_all), seg.view(-1,1).squeeze(),mode='seg')
            loss.backward()
            opt.step()
...
def test(args, io):
    ...
    if args.model == 'dgcnn':
        model = DGCNNCaps_part(args, seg_num_all).to(device)
    else:
        raise Exception("Not implemented")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='dgccaps_part', metavar='N',
                        choices=['dgcnn','dgccaps_part'],
